  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-400 text-sm mt-1">Selamat datang kembali! Berikut ringkasan performa toko Anda hari ini.</p>
      </div>

      {/* Stats Grid */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {stats.map((stat, idx) => (
          <div key={idx} className="bg-slate-900 border border-gray-800 rounded-xl p-6 flex flex-col justify-between">
            ...
            <div className="flex items-center gap-2 mt-4 text-xs">
              <span className={`flex items-center gap-1 font-semibold ${stat.isPositive ? 'text-teal-400' : 'text-red-400'}`}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-slate-900 border border-gray-800 rounded-xl p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <button className="text-teal-400 text-xs hover:underline">Lihat Semua</button>
          </div>
              ...
              <tbody className="divide-y divide-gray-800/50">
                {transactions.map((trx, idx) => (
                  <tr key={idx} className="text-gray-300 hover:bg-slate-850/40">
                    <td className="py-3 font-semibold text-teal-400">{trx.id}</td>
                    <td className="py-3">
                      <div className="font-medium text-white">{trx.customer}</div>
                      <div className="text-xs text-gray-500">{trx.email}</div>
                    </td>
                    <td className="py-3">{trx.product}</td>
                    <td className="py-3 font-medium text-white">{trx.amount}</td>
                    <td className="py-3">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${
                        trx.status === 'Sukses' ? 'bg-teal-500/10 text-teal-400 border-teal-500/20' :
                    ...
                    <td className="py-3 text-right text-gray-500">{trx.date}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-slate-900 border border-gray-800 rounded-xl p-6 space-y-6">
          <div>
            <h2 className="text-lg font-bold text-white mb-2">Informasi Server</h2>
            <p className="text-gray-400 text-sm">Status server database dan endpoint internal.</p>
          </div>

          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-gray-950/50 rounded-lg border border-gray-850">
              <span className="text-sm text-gray-400">Database MySQL Connection</span>
          ...
          <div className="pt-2">
            <div className="p-4 bg-teal-950/20 border border-teal-500/20 rounded-lg text-teal-400 text-xs leading-relaxed">
              <strong>Info Pembelajaran:</strong> Anda telah memigrasikan Express ke Next.js Full-Stack. Halaman ini di-render secara server-side secara default, namun komponen interaktif dapat dengan mudah menggunakan hooks seperti `useState` di client.
            </div>
          </div>
        </div>
