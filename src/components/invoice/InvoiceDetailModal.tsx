...
export function InvoiceDetailModal({ item, onClose }: InvoiceDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden text-white flex flex-col max-h-[90vh]">
        {/* Modal Header */}
        <div className="p-5 border-b border-slate-800 flex items-center justify-between shrink-0">
          <h3 className="font-bold text-lg">Detail Invoice Penjualan</h3>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6" id="invoice-print-area">
          <div className="space-y-6 text-slate-300">
            ...
            <div className="text-center pb-6 border-b border-slate-800 space-y-1">
              <h2 className="text-2xl font-black tracking-wider text-white">BursaAMVBG</h2>
              <p className="text-xs text-slate-400">Invoice Penjualan Inventori</p>
              <p className="text-xs text-slate-500 font-mono mt-2">ID: {item.id}</p>
            </div>
            ...
            {/* Product Detail */}
            <div className="border border-slate-800 rounded-2xl bg-slate-950 p-4 space-y-3">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-800 pb-2">
                Item Terjual
              </p>
            ...
            <div className="space-y-2 border-t border-slate-800 pt-4">
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-400 font-medium">Subtotal Jual</span>
                <span className="font-bold text-white">
                  Rp {item.revenue.toLocaleString("id-ID")}
                </span>
              </div>
              <div className="flex justify-between items-center text-sm border-t border-slate-800/40 pt-2">
                <span className="text-slate-400 font-medium">Keuntungan Bersih</span>
        ...
        <div className="p-4 bg-slate-950/80 border-t border-slate-800 flex gap-3 shrink-0">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-2.5 bg-slate-800 hover:bg-slate-700 active:scale-[0.98] transition font-semibold rounded-xl text-sm"
          >
            Tutup
          </button>
          <button
